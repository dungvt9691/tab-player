...
(function() {
  ...
  $.awesomeCropper = function(inputAttachTo, options) {
    ...
    settings = {
      width: 100,
      height: 100,
      matchRegx: /\.(jpg|png|gif|jpeg)$/mi,
      acceptExtensions: ".jpeg, .jpg, .JPEG, .JPG, .png, .PNG, .gif, .GIF",
      debug: false,
      success: function(){
    ...
    $container = div().insertAfter($('body')).addClass('awesome-cropper');
    ...
    $fileSelect.attr("accept", settings.acceptExtensions)

    if(typeof settings.inputClass != "undefined"){
      $fileSelect.addClass(settings.inputClass)
    }
    ...
    $sourceIm = image();
    $applyButton = a(settings.applyButton).addClass('btn yes');
    $cancelButton = a(settings.cancelButton).addClass('btn').attr({
      'data-dismiss': "modal"
    });
    $imagesContainer = div().append(div().addClass('modal-dialog modal-lg').append(div().addClass('modal-content').append(div().addClass('modal-body').append(div().addClass('col-md-12').append($sourceIm)).append(div().addClass('clearfix')), div().addClass('modal-footer').append(div().addClass('').append($cancelButton).append('<span class="separator"> • </span>').append($applyButton))))).addClass('modal').attr({
      role: 'dialog'
    });
    ...
    removeLoading = function() {
      $imagesContainer.on('shown.bs.modal', function() {
        $("#popup-upload-files").modal('hide')
        return setAreaSelect($sourceIm);
      }).on('hidden.bs.modal', function() {
        $("#popup-upload-files").modal('show')
        return cleanImages();
      }).modal();
    ...
    fileAllowed = function(name) {
      var res;
      res = name.match(settings.matchRegx);
      if (!res) {
        showMessage(settings.invalidExtensionsMessage + " " + settings.acceptExtensions, 'error');
        return false;
      } else {
        return true;
      }
    ...
    saveCrop = function() {
      var result;
      drawImage($sourceIm, $x1, $y1, $x2, $y2);
      result = $cropSandbox.get(0).toDataURL();
      options.element.html("<img src='"+result+"' />")
      settings.success()
      $inputAttachTo.val(result);
      if(typeof settings.formElement != 'undefined'){
